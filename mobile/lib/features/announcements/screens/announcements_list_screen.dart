...
class _AnnouncementsListScreenState extends ConsumerState<AnnouncementsListScreen> {
  ...
  Widget _buildAnnouncementCard(BuildContext context, AnnouncementModel notice, bool isParent) {
    ...
    if (notice.isUrgent) {
      ...
      iconBg = AppColors.priorityImportantBg;
      iconColor = AppColors.priorityImportantText;
      iconData = Icons.description_outlined;
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              Navigator.pop(ctx);
              final error = await ref.read(announcementsProvider.notifier).deleteAnnouncement(notice.id);
              messenger.showSnackBar(
                SnackBar(
                  content: Text(error ?? 'Announcement deleted successfully'),
  ...
  void _showAddAnnouncementDialog(BuildContext context) {
    ...
    final isTeacher = user?.role.isTeacher ?? false;
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                final error = await ref.read(announcementsProvider.notifier).createAnnouncement(
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(error ?? '"${titleCtrl.text.trim()}" posted successfully!'),
                      backgroundColor: error == null
                          ? AppColors.statusActiveText
                          : AppColors.statusOverdueText,
                    ),
